...
for idx, (g_L, color) in enumerate(zip(g_L_values, colors)):
    ...
    axes[idx].set_title(f'g_L = {g_L} μS')
...
# Set up recording (record membrane potential only)
# Note: Check neuron documentation for available variables
recorder = neun.Recorder(neuron, variables=['V'])
...
# Plot recorded voltage
fig, ax = plt.subplots(figsize=(10, 6))

ax.plot(time, data['V'], 'b-', linewidth=1.5)
ax.set_ylabel('Membrane Potential (mV)')
ax.set_xlabel('Time (ms)')
ax.set_title('Recorded Membrane Potential', fontsize=14)
...
- ✅ How to install and set up Neun
- ✅ Basic architecture and concepts
- ✅ Creating and simulating single neurons
- ✅ Working with populations
...
1. **Neun workflow**: Create → Define inputs → Simulate → Analyze
